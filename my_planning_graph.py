...
class PlanningGraph():
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for prec1 in node_a1.parents:
            for prec2 in node_a2.parents:
                if prec1.is_mutex(prec2):
                    return True  
        return False
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        if(node_s1.literal == expr('~{}'.format(node_s2.symbol))):
            return True
        if(node_s2.literal == expr('~{}'.format(node_s1.symbol))):
            return True
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in goals:
            i = 0
            while True:
                if goal in [n.literal for n in self.s_levels[i]]:
                     level_sum += i
                     break
                i += 1
